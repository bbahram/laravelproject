...
<body>
    ...
    <div id="Provider" class="tabcontent">
        <form method="POST" action={{route('providerUpdate',$provider->id)}} style="border:1px solid #ccc">

            @csrf
            <div class="container">
                <div style="text-align: center">
                    <h1>تغییر اطلاعات فراهم کننده</h1>
                    <p>فرم زیر را تکمیل کنید.</p>
                </div>
                <hr>

                <label for="name"><b>نام فراهم کننده</b></label>
                <input type="text" placeholder="نام اتاق را وارد کنید." name="name" value="{{$provider->name}}" required >

                <label for="email"><b>ایمیل</b></label>
                <input type="text" placeholder="توضیحات را وارد کنید." name="email" value="{{$provider->email}}" required>
